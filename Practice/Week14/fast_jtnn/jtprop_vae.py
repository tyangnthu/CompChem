...
class JTPropVAE(nn.Module):
    ...
    def forward(self, mol_batch, beta=0,wp=2.5,stats=False):
        x_batch, x_jtenc_holder, x_mpn_holder, x_jtmpn_holder,prop_batch = mol_batch
        # batch_size = len(x_batch)
        # prop_batch, count,zeff = prop_batch
        ...
        # z_mean = torch.cat([z_tree_vecs, z_mol_vecs], dim=1)
        ...
        idxes_to_use = [2.0000 not in label for label in prop_label_raw]
        if True in idxes_to_use:
            ...
            all_vec = z_mean[idxes_to_use]
            prop_pred = self.propNN(all_vec)
            # prop_pred_act = 1/(1 + torch.exp(-prop_pred.clone()*4+2))
            if stats is not False:
                # print('prop_pred is ' + str(elf.propNN(all_vec).squeeze()))
                # print('std is ' + str(torch.Tensor(std)))
                prop_pred_unnorm = prop_pred * torch.Tensor(stats[1]).cuda() + torch.Tensor(stats[0]).cuda()
                prop_label_unnorm = prop_label * torch.Tensor(stats[1]).cuda() + torch.Tensor(stats[0]).cuda()
                prop_loss_unnorm = ((prop_pred_unnorm - prop_label_unnorm) ** 2).mean(axis=0) ** 0.5
                prop_loss_unnorm = prop_loss_unnorm.detach().cpu().numpy().tolist()
                # print('prop_loss_unnorm is %s' %(prop_loss_unnorm))
                # prop_loss_ = (prop_pred_ - prop_label_) ** 2
                # prop_loss = self.prop_loss(.squeeze() * torch.Tensor(std), prop_label.squeeze() * torch.Tensor(std))
            prop_loss = self.prop_loss(prop_pred.squeeze(-1), prop_label)
            loss = word_loss + topo_loss + assm_loss + beta * kl_div + wp * prop_loss
            p_loss = prop_loss.item()
            # print('True, p_loss is %s' %(p_loss))
        else:
            loss = word_loss + topo_loss + assm_loss + beta * kl_div
            # prop_loss_unnorm = [float('nan')] * len(prop_label_raw[2])
            p_loss = float('nan')
        
        return loss, kl_div.item(), word_acc, topo_acc, assm_acc, p_loss
